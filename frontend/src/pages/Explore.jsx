...
const ExplorePage = () => {
    ...
    const applyFilters = () => {
        let filtered = venues;
        if (filters.type) {
            filtered = filtered.filter(venue => venue.type.toLowerCase() === filters.type.toLowerCase());
        }
        if (filters.location) {
            filtered = filtered.filter(venue => venue.address.toLowerCase().includes(filters.location.toLowerCase()));
        }
                ...
                <div className="flex">
                    ...
                    <div className="w-full md:w-1/4 bg-white p-4 rounded-lg shadow-lg">
                        ...
                        <div className="mb-4">
                            ...
                            <div>
                                <input type="checkbox" id="futsal" name="type" value="Futsal" onChange={handleFilterChange} className="mr-2" />
                                <label htmlFor="futsal">Futsal</label>
                            </div>
                            <div>
                                <input type="checkbox" id="cricket" name="type" value="Cricket" onChange={handleFilterChange} className="mr-2" />
                                <label htmlFor="cricket">Cricket</label>
                            </div>
                        </div>
                        <div>
                            <h3 className="text-lg font-medium">Location</h3>
                            <select name="location" onChange={handleFilterChange} className="w-full p-2 border rounded-lg">
